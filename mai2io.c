...
HRESULT mai2_io_init(void) {
    ...
    dprintf("[UTS IO] Running in %s\n", filename);

    if (_stricmp(filename, "Sinmai.exe") == 0) {
        dprintf("[UTS IO] Initializing touch overlay for Sinmai.exe\n");
        touch_overlay_init();
    }

    dprintf("[UTS IO] Initialization complete.\n");
    return S_OK;
}
...
HRESULT mai2_io_touch_init(mai2_io_touch_callback_t callback) {
    dprintf("[UTS IO] Setting callback\n");
    _callback = callback;
    return S_OK;
}
...
#if 0
    dprintf("[UTS IO] Mai2 touch side %c: set sensor %s sensitivity to %d\n", bytes[1], sensor_to_str(bytes[2]), bytes[4]);
#endif
    return;
}
...
void mai2_io_touch_update(bool player1, bool player2) {
    if (mai2_io_cfg.debug_input_1p) {
        if (player1 && mai2_io_touch_1p_thread == NULL) {
            dprintf("[UTS IO] Enabling 1P thread\n");
            ...
            dprintf("[UTS IO] Disabling 1P thread\n");
            ...
            dprintf("[UTS IO] 1P thread stopped\n");
        }
    }

    if (mai2_io_cfg.debug_input_2p) {
        if (player2 && mai2_io_touch_2p_thread == NULL) {
            dprintf("[UTS IO] Enabling 2P thread\n");
            touch_overlay_enable_2p(true);
            ...
            dprintf("[UTS IO] Disabling 2P thread\n");
            ...
            dprintf("[UTS IO] 2P thread stopped\n");
        }
    }
}
...
static unsigned int __stdcall mai2_io_touch_1p_thread_proc(void *ctx) {
    ...
    while (!mai2_io_touch_1p_stop_flag) {
        ...
        touch_overlay_get_state(0, overlay_state);
        for(int i=0; i<7; i++) {
            state[i] |= overlay_state[i];
        }
...
static unsigned int __stdcall mai2_io_touch_2p_thread_proc(void *ctx) {
    ...
    while (!mai2_io_touch_2p_stop_flag) {
        ...
        touch_overlay_get_state(1, overlay_state);
        for(int i=0; i<7; i++) {
            state[i] |= overlay_state[i];
        }
...
HRESULT mai2_io_led_init(void) { 
    dprintf("[UTS IO] Initializing LEDs\n");
    return S_OK; 
}
...
void mai2_io_led_set_fet_output(uint8_t board, const uint8_t *rgb) {
    ...
    dprintf("[UTS IO] LED %dP: BodyLed brightness: %d%%\n", player,
            (rgb[0] * 100) / 255);
    dprintf("[UTS IO] LED %dP: ExtLed brightness: %d%%\n", player,
            (rgb[1] * 100) / 255);
    dprintf("[UTS IO] LED %dP: SideLed brightness: %d%%\n", player,
            (rgb[2] * 100) / 255);
#endif
    return;
}
...
void mai2_io_led_dc_update(uint8_t board, const uint8_t *rgb) {
    ...
    uint8_t player = board + 1;
    for (int i = 0; i < 10; i++) {
        dprintf("[UTS IO] LED %dP: LED %d: %02X %02X %02X Speed: %02X\n", player,
